...
//            nestedScrollView = (NestedScrollView) wrapperContent.getChildAt(0);
...
//            nestedScrollView.getLayoutParams().height=wrapperContent.getHeight();
            isSetted = true;
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
                ...
                int creteria = mScreenHeight / 10;//滑动多少距离
                if (ispageOne) {
                    if (scrollY <= creteria) {
                        //显示菜单
